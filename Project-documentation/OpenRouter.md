...
*   **Transfer Steps for Client or IT Staff:**
    1. Log into the OpenRouter dashboard.
    2. Navigate to the Workspace settings.
    3. Invite Dr Vong or the BioTective IT team as members with administrative privileges.
    4. Alternatively generate a new API key under the BioTective account and update the backend environment variables.
...
*   **Day 2 Operations:** To switch to a different AI provider or model the development team simply needs to update the `LLM_MODEL` environment variable in Vercel without changing any backend code.