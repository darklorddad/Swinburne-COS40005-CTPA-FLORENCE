...
The manual focuses only on implemented admin-side functionality:
...
The Admin Portal is intended for authorised administrative users.

Administrators should have:

1. An authorised admin email address.
2. A valid password.
3. Permission to manage operational records such as patients, clinicians and organisations.
...
- **Left sidebar:** Persistent navigation for Dashboard, Patients, Clinicians, Organisations, Data Simulator, Settings and Logout.
...
3. The system clears the admin authentication state, signs out and returns the user to the login flow.

![alt text](admin-logout.png)
...
- **Total Patients:** Number of patient profiles.
...
The system registers the account and refreshes the patient list.

![alt text](register-patient.png)

### 6.4. Opening Patient Details

1. In the Patient Directory, locate the patient.
2. Select the edit icon (pencil) in the *Actions* column.
...
2. Select the edit icon (pencil) in the *General Information* section.
...
The updated patient details are saved and the patient list is refreshed.

![alt text](edit-patient-profile.png)
...
This action permanently deletes the patient profile.
...
The system registers the account and refreshes the clinician list.

![Register clinician](register-clinician.png)

### 7.4. Editing a Clinician

1. In the Clinician Directory, select the edit icon (pencil) for a clinician.
...
When a clinician is deleted, assigned patients are first unassigned to avoid broken assignments. The clinician profile is then deleted.
...
The Organisations module manages healthcare facilities, clinics, hospitals, health centres, labs and other organisations.

### 8.1. Opening the Organisations Directory
...
2. Select the view icon (eye).
3. Review the Organization Detail screen.
...
- The edit icon (pencil) in the Organizations directory.
- The *Edit Organization* button in the Organization Detail screen.
...
## 11. Safety Checklist for Administrators
