...
*Note: Variables marked as "Needs Attention" in the Vercel dashboard (such as API keys) must be verified and updated with live production keys upon handover.*
...
*   **web (Flutter Web):** Hosts the Admin and Clinician dashboards. Root directory: `florence/platform_service`. Framework: Custom (Build script: `bash build_web.sh` and Output directory: `build/web`).
...
*   **Upgrading:** If the platform moves to a live production environment with real patients the client must upgrade to the Vercel Pro tier to increase limits, access advanced deployment protection and enable commercial usage.
