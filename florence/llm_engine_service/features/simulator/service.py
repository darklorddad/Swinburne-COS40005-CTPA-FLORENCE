...
class SimulatorService:
    ...
    async def generate_patient_data(self, request: SimulatorRequest) -> SimulatedMonth:
        ...
        2. Use Malaysian food contexts (e.g. Nasi Lemak, Roti Canai, Laksa, Chicken Rice) and realistic Malaysian names where appropriate.
        3. All glucose and cholesterol values MUST be in mmol/L.
        4. Generate the `profile` object with realistic baseline demographics, emergency contacts, and AI text matching the scenario.
        5. `day_offset` in `days` must start at 0 (today) and go exactly up to {request.days - 1}.
        6. Daily Data: Generate exactly 3 meals and Blood Pressure for EVERY day. Include activity only if it aligns with the scenario.
        
        7. LONG-TERM VITALS (CRITICAL FOR CHARTS):
