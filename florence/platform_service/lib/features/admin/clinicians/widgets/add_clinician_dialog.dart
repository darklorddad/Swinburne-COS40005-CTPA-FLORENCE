...
class AddClinicianDialog extends ConsumerStatefulWidget {
  const AddClinicianDialog({super.key});

  @override
  ConsumerState<AddClinicianDialog> createState() => _AddClinicianDialogState();
}

class _AddClinicianDialogState extends ConsumerState<AddClinicianDialog> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedOrgId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select an organisation'), backgroundColor: AdminTheme.error),
      );
      return;
    }
    ...
    try {
      ...
      await repository.registerNewClinician({
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Register New Clinician', style: Theme.of(context).textTheme.headlineMedium),
                        const SizedBox(height: 4),
                        const Text('Enter the clinician details to generate an account.', style: TextStyle(color: AdminTheme.outline)),
                      ],
                    ),
                    IconButton(
                ...
                orgsAsync.when(
                  data: (orgs) {
                    return DropdownButtonFormField<int>(
                      ...
                      decoration: _inputDecoration('Organisation', Icons.business_outlined),
                      items: orgs.map((org) {
                        return DropdownMenuItem<int>(value: org.id, child: Text(org.name));
                      }).toList(),
                      onChanged: (val) => setState(() => _selectedOrgId = val),
                      validator: (val) => val == null ? 'Organisation is required' : null,
                    );
                  },
                ...
                _buildTextField(
                  ...
                  helperText: 'Clinician can change this later',
